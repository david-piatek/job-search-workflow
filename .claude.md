# Job Scraper Application - Documentation Projet

## Vue d'ensemble

Job search workflow automation platform with multiple applications:

### Applications

1. **Job Scraper** - Job offers scraping application
   - Frontend: Vite + Svelte
   - Backend: NestJS + TypeORM + Bull Queue
   - Features: Dynamic scraper generation, job search automation

2. **Letter Generator** - CLI tool for generating personalized cover letters
   - Technology: Node.js + Puppeteer
   - Features: HTML template hydration, PDF generation, CSS styling
   - Input: JSON data file + HTML template
   - Output: Professional styled PDF letters
   - Supports: QR codes, images, custom fonts, CSS styling

3. **QR Code Generator** - Service for generating QR codes
   - Technology: Node.js + qrcode library
   - Features: PNG/SVG generation, customizable colors, batch processing
   - Integration: Works with Letter Generator for inline QR codes
   - Supports: Data URLs, file output, elegant styling preset

4. **Static Site Generator** - Générateur de sites de présentation
   - Technology: Node.js + HTML/CSS
   - Features: Sites élégants générés depuis JSON, responsive design
   - Sections: Démarche, workflow, matching, CV iframe, stats
   - Deployment: GitHub Pages, Netlify, Vercel ready

5. **Application Workflow** - Workflow complet de candidature
   - Technology: NestJS + TypeORM + Puppeteer
   - Features: Orchestration complète génération de candidature
   - Workflow: Site HTML → Site PDF → QR Code → Lettre PDF avec QR
   - Backend-orchestrated avec polling frontend
   - Self-hosted sites via endpoint dédié
   - Progress tracking en temps réel

### Infrastructure

- Docker & Kubernetes for containerization
- Helm charts for deployment
- ArgoCD for GitOps
- n8n for workflow automation

## Architecture

### Backend (NestJS)

**Modules:**

- `scrapers`: Gestion et génération dynamique de scrapers
- `jobs`: Stockage et récupération des offres scrapées
- `generators`: Services de génération (Site, QR, Letter)
- `workflow`: Orchestration complète du workflow de candidature
- `hosting`: Service de sites statiques self-hosted

**Services clés:**

- `ScraperGeneratorService`: Génère le code des scrapers automatiquement
- `ScraperExecutorService`: Exécute les scrapers et sauvegarde les résultats
- `ScraperProcessor`: Traite les jobs de scraping via Bull Queue
- `WorkflowService`: Orchestre le workflow complet de candidature (Site → QR → Letter)
- `SiteService`: Génération de sites statiques (HTML + PDF)
- `QrService`: Génération de QR codes avec styles personnalisés
- `LetterService`: Génération de lettres PDF avec QR codes intégrés
- `HostingService`: Service de sites statiques self-hosted

**Technologies:**

- TypeORM pour PostgreSQL
- Bull pour les queues Redis
- Puppeteer + Cheerio pour le scraping
- Swagger pour la documentation API

### Frontend (Svelte)

**Composants principaux:**

- `ScraperSelector`: Sélection et création de scrapers
- `JobsList`: Affichage des résultats scrapés
- `Stats`: Dashboard de statistiques

**Features:**

- API client avec Axios
- Storybook pour documentation composants
- Tests E2E avec Playwright

### Infrastructure

**Docker Compose:**

- PostgreSQL 16
- Redis 7
- Backend NestJS
- Frontend Svelte/Nginx

**Kubernetes (Helm):**

- Deployments: backend, frontend
- StatefulSets: postgres, redis
- Services, Ingress, PVC
- HPA (optionnel)

**ArgoCD:**

- GitOps pour déploiement automatique
- Sync automatique des changements
- Health checks et rollbacks

### Workflow n8n

Pipeline automatisé:

1. Schedule (cron toutes les 6h)
2. Récupération des scrapers
3. Exécution séquentielle
4. Sauvegarde JSON locale
5. Backup GitHub dans `/backup`
6. Notification Slack

## Standards Appliqués

### Structure Monorepo

```
apps/
  backend/              # NestJS API (job scraper)
  frontend/             # Svelte SPA (job scraper UI)
  letter-generator/     # CLI tool for PDF letter generation
    src/                # Source code
      index.js          # Standard generator
      index-qr.js       # Generator with QR service integration
      qr-service.js     # QR code service wrapper
    templates/          # Letter templates
    output/             # Generated PDFs
    test/               # Tests
  qr-generator/         # QR code generation service
    src/                # Source code
      index.js          # CLI
      generator.js      # QR generation logic
    output/             # Generated QR codes
    test/               # Tests
  static-site/          # Static site generator
    src/                # Source code
      index.js          # Generator CLI
    templates/          # HTML templates
      presentation.html # Main template
      synthwave.html    # Synthwave template
    data/               # JSON data files
    output/             # Generated sites
    assets/             # Images, CSS
hosted-sites/           # Self-hosted static sites (served by backend)
cloud/
  docker/               # Docker Compose and Dockerfiles
    Dockerfile.backend
    Dockerfile.frontend
    Dockerfile.letter-generator
    Dockerfile.qr-generator
  helm/                 # Kubernetes charts
  argocd/               # GitOps config
n8n/                    # Workflow automation
  template-lettre.txt   # Letter template
```

### Docker

- Multi-stage builds pour optimisation
- Images Alpine pour légèreté
- Health checks configurés
- Volumes pour persistence
- Utilisation de `docker compose` (v2) au lieu de `docker-compose`
- Centralisation dans `cloud/docker/`

### Helm Charts

- Templates modulaires
- Values pour différents environnements
- Secrets management
- Resource limits/requests
- Probes (liveness/readiness)

### Taskfile

Commandes standardisées:

- `dev`: Développement
- `build`: Build production
- `test`: Tests (unit + e2e)
- `lint`: Linting
- `docker:*`: Gestion Docker
- `helm:*`: Déploiement K8s

### Tests

**Backend:**

- Tests unitaires: Jest
- Tests e2e: Supertest
- Coverage configuré

**Frontend:**

- Tests unitaires: Vitest
- Tests e2e: Playwright
- Tests composants: Storybook

### Git Hooks

- **pre-commit**: lint-staged
  - Prettier
  - ESLint

- **pre-push**:
  - Tous les tests

### Outils d'Analyse

- **Swagger**: Documentation API auto-générée
- **Storybook**: Catalogue de composants UI
- **TypeScript**: Typage strict
- **ESLint + Prettier**: Code quality

## Fonctionnement

### Génération Dynamique de Scrapers

1. Frontend envoie nom + URL du site
2. Backend vérifie si scraper existe en DB
3. Si non, `ScraperGeneratorService` génère le code
4. Code générique avec sélecteurs CSS communs
5. Sauvegarde en DB avec metadata
6. Retour du scraper au frontend

### Exécution de Scraping

1. Frontend demande exécution
2. Backend ajoute job à la queue Redis
3. `ScraperProcessor` récupère le job
4. Exécution du code scraper (Puppeteer)
5. Parsing HTML avec Cheerio
6. Sauvegarde JSON + PostgreSQL
7. Mise à jour stats d'exécution

### Déploiement GitOps

1. Push vers GitHub
2. ArgoCD détecte changement
3. Sync automatique Helm chart
4. Rollout K8s progressif
5. Health checks
6. Rollback automatique si erreur

### Workflow n8n

1. Trigger schedule (cron)
2. GET `/scrapers` → liste
3. Pour chaque scraper:
   - POST `/scrapers/execute`
   - Wait 30s
   - GET `/jobs?source={scraper}`
4. Save JSON local + GitHub
5. Notification Slack

## Configuration Environnements

### Développement

**Lancement rapide:**

```bash
# Nettoyer les conteneurs existants si nécessaire
docker compose -f cloud/docker/compose.yaml down

# Lancer tous les services de développement
task dev
```

**Services démarrés:**

- ✅ **Frontend**: http://localhost:5174 (Svelte + Vite)
  - Note: Port auto-incrémenté si 5173 occupé
- ✅ **Backend**: http://localhost:3001 (NestJS)
  - API Hello: http://localhost:3001/api/hello
  - Swagger: http://localhost:3001/api
- ✅ **PostgreSQL**: localhost:5432 (user: admin, password: admin123, db: jobscraper)
- ✅ **Redis**: localhost:6379

**Lancement individuel:**

```bash
task dev:frontend  # Frontend seul (port 5174)
task dev:backend   # Backend seul (port 3001)
```

**Prérequis développement local:**

- Créer un fichier `.env` dans `apps/backend/` basé sur `.env.example`
- Port backend configurable via variable `PORT` (défaut: 3001 pour éviter conflits)
- Services Docker requis: PostgreSQL (port 5432), Redis (port 6379)

**Configuration Backend (.env):**

```env
NODE_ENV=development
PORT=3001
DATABASE_URL=postgresql://admin:admin123@localhost:5432/jobscraper
REDIS_URL=redis://localhost:6379
DATA_DIR=./data
```

**Démarrage services infrastructure:**

```bash
# Démarrer PostgreSQL et Redis
docker compose -f cloud/docker/compose.yaml up postgres redis -d

# Vérifier que les services sont up et healthy
docker ps | grep job-scraper
```

### Docker Local

```bash
task docker:up
# Avec PostgreSQL + Redis + Backend + Frontend
```

### Production K8s

```bash
# Via Helm
task helm:install

# Via ArgoCD (GitOps)
kubectl apply -f argocd/application.yaml
```

## Points d'Attention

### Sécurité

- Secrets en K8s Secrets (pas en values.yaml plain)
- CORS configuré sur backend
- Validation des inputs (class-validator)
- Rate limiting recommandé (production)

### Performance

- Bull Queue pour async processing
- Redis cache
- Index DB sur colonnes fréquentes
- HPA pour scaling automatique

### Monitoring

- Logs structurés (JSON)
- Health endpoints (`/api`)
- Prometheus metrics (à ajouter)
- Grafana dashboards (à ajouter)

### Backup

- Données jobs en PostgreSQL
- Fichiers JSON en PVC K8s
- Backup GitHub via n8n
- Snapshots DB recommandés

## Prochaines Étapes

1. Ajouter plus de scrapers prédéfinis
2. Améliorer génération automatique (IA)
3. Implémenter authentification API
4. Ajouter métriques Prometheus
5. CI/CD pipeline (GitHub Actions)
6. Documentation utilisateur détaillée
7. Tests de charge
8. Monitoring avancé (Datadog/New Relic)

## Commandes Utiles

```bash
# Dev
task dev

# Tests
task test
task test:letter

# Build
task build

# Docker
task docker:build
task docker:up

# Letter Generator
task letter:generate                    # Generate with example data
task letter:generate:elegant            # Generate elegant Christmas letter
task letter:generate:qr                 # Generate with local QR code
task letter:generate:custom \           # Generate with custom data
  TEMPLATE=path/to/template.txt \
  DATA=path/to/data.json \
  OUTPUT=path/to/output.pdf
task letter:docker:build                # Build Docker image
task letter:docker:run                  # Run in Docker

# QR Code Generator
task qr:generate URL="https://example.com"     # Generate QR code
task qr:generate:elegant URL="https://..."     # Generate elegant QR code
task qr:docker:build                           # Build Docker image
task qr:docker:run URL="https://..."           # Run in Docker

# Static Site Generator
task site:generate                             # Generate presentation site
task site:generate:custom DATA=data/file.json  # Generate with custom data
task site:serve                                # Serve locally
task site:open                                 # Open in browser

# K8s
task helm:install
kubectl get pods -n job-scraper

# Logs
task docker:logs
kubectl logs -n job-scraper -l app=backend -f

# Storybook
task storybook

# Swagger
open http://localhost:3000/api
```

## Technologies Complètes

**Frontend:**

- Svelte 4
- Vite 5
- Axios
- Storybook 7
- Playwright
- Vitest

**Backend:**

- NestJS 10
- TypeORM
- Bull
- Puppeteer
- Cheerio
- Swagger
- Jest

**Infrastructure:**

- Docker & Docker Compose
- Kubernetes
- Helm 3
- ArgoCD
- n8n
- PostgreSQL 16
- Redis 7

**DevOps:**

- Task (go-task)
- Husky
- lint-staged
- Prettier
- ESLint

## Organisation Cloud

L'ensemble des ressources cloud est centralisé dans le répertoire `cloud/`:

- **`cloud/docker/`**:
  - `compose.yaml` (v2 naming convention)
  - `Dockerfile.backend`
  - `Dockerfile.frontend`

- **`cloud/helm/`**:
  - Charts Kubernetes pour déploiement

- **`cloud/kubernetes/`**:
  - Configuration GitOps

### Conventions

- Utiliser `docker compose` (v2) au lieu de `docker-compose` (v1)
- Fichier nommé `compose.yaml` (convention moderne) au lieu de `docker-compose.yml`
- Paths relatifs depuis la racine du projet
- Centralisation cloud pour faciliter la maintenance

## CI/CD

### GitHub Actions (CI)

Pipeline de Continuous Integration sur GitHub:

- **Lint**: ESLint et Prettier sur backend et frontend
- **Tests Backend**: Tests unitaires et e2e avec PostgreSQL/Redis
- **Tests Frontend**: Tests unitaires et e2e
- **Build**: Build de production backend et frontend
- **Docker Build**: Validation des images Docker
- **Trigger GitLab**: Déclenche le CD GitLab si CI passe

### GitLab CI/CD (CD)

Pipeline de Continuous Deployment sur GitLab:

- **Build**: Construction et push des images Docker au registry
- **Deploy Staging**: Déploiement auto sur branche `develop`
- **Deploy Production**: Déploiement manuel sur branche `main`
- **Verify**: Health checks post-déploiement

### Workflow

1. Push sur GitHub → CI s'exécute
2. CI passe → Trigger GitLab CD
3. GitLab build les images Docker
4. GitLab déploie sur Kubernetes
5. Verification du déploiement

### Configuration Requise

**GitHub Secrets:**

- `GITLAB_TRIGGER_TOKEN`
- `GITLAB_TRIGGER_URL`

**GitLab Variables:**

- `CI_REGISTRY`, `CI_REGISTRY_USER`, `CI_REGISTRY_PASSWORD`
- `KUBE_CONFIG_STAGING`, `KUBE_CONFIG_PROD`

Voir `SETUP_CI_CD.md` pour la configuration complète.

## Letter Generator Usage

### Templates Disponibles

#### 1. Template Standard (`letter-template.html`)

Template professionnel basique et épuré.

```bash
task letter:generate
```

#### 2. Template Élégant de Noël (`letter-christmas-elegant.html`)

Template premium avec design festif sophistiqué :

- ✍️ Polices manuscrites élégantes (Cormorant Garamond, Playfair Display, Great Vibes)
- 🎨 Palette de couleurs raffinée (bordeaux, or, crème)
- 🎄 Ornements festifs subtils et professionnels
- 📐 Typographie soignée et mise en page élégante
- 🌟 Signature manuscrite stylisée

**Usage:**

```bash
task letter:generate:elegant
```

**Parfait pour:**

- Candidatures de fin d'année
- Communications premium
- Networking période de fêtes
- Lettres de motivation executives

### Génération Rapide

```bash
# Template standard
task letter:generate

# Template élégant Noël
task letter:generate:elegant

# Template personnalisé
task letter:generate:custom \
  TEMPLATE=path/to/template.html \
  DATA=path/to/data.json \
  OUTPUT=path/to/output.pdf
```

### Docker Usage

```bash
# Build image
task letter:docker:build

# Generate letter
task letter:docker:run
```

### Variables Template Standard

```json
{
  "Date": "4 décembre 2025",
  "matching-description": "Description...",
  "qr-code": "<img src='URL'>",
  "cv-link": "<a href='URL'>Texte</a>",
  "contact-info": "contact@example.com"
}
```

### Variables Template Élégant

```json
{
  "sender-name": "Nom Prénom",
  "sender-address": "Adresse",
  "sender-city": "Ville Code Postal",
  "Date": "Date",
  "subject": "Objet de la lettre",
  "greeting": "Madame, Monsieur",
  "intro-paragraph": "Premier paragraphe...",
  "second-paragraph": "Deuxième paragraphe...",
  "matching-description": "Ce qui vous a interpellé...",
  "closing-paragraph": "Paragraphe de conclusion...",
  "wishes": "Vœux de fin d'année...",
  "closing-formula": "Formule de politesse",
  "signature-text": "Signature",
  "qr-code": "<img src='QR code URL'>",
  "cv-link": "<a href='URL'>Lien portfolio</a>",
  "contact-info": "Informations de contact complètes"
}
```

### Typographie Template Élégant

**Polices Google Fonts:**

- **Cormorant Garamond**: Corps de texte (serif élégant)
- **Playfair Display**: Titres et emphase
- **Great Vibes**: Signature (manuscrit script)

**Palette de Couleurs:**

- Texte principal: `#2c3e50` (bleu-gris foncé)
- Accent or: `#8b6f47` (bronze chaud)
- Accent bordeaux: `#a07855` (brun-or élégant)
- Fond: `#fdfbf7` (crème chaud)
- Bordures: `#e8dcc8` (beige doux)

### Personnalisation Templates

Templates = fichiers HTML avec :

- Support CSS3 complet
- Intégration Google Fonts
- Styles print-optimized (`@page`, `@media print`)
- Variables: `{{variable-name}}`
- Support images/QR codes
- HTML dans les données JSON possible

## QR Code Generator Usage

### Génération Simple

```bash
# QR code standard
task qr:generate URL="https://github.com/davidpiatek"

# QR code élégant (couleurs template)
task qr:generate:elegant URL="https://github.com/davidpiatek"
```

### CLI Direct

```bash
cd apps/qr-generator

# PNG standard
node src/index.js generate \
  -d "https://example.com" \
  -o output/qr.png \
  --width 300 \
  --dark "#000000" \
  --light "#FFFFFF"

# Style élégant
node src/index.js elegant \
  -d "https://example.com" \
  -o output/elegant-qr.png

# Data URL (base64 inline)
node src/index.js data-url \
  -d "https://example.com" \
  --width 200

# Batch generation
node src/index.js batch -i batch-config.json
```

### Intégration avec Letter Generator

**Méthode 1: Utiliser qr-code-url dans JSON**

```json
{
  "qr-code-url": "https://github.com/davidpiatek",
  "cv-link": "https://github.com/davidpiatek"
}
```

Puis générer avec:

```bash
task letter:generate:qr
```

Le QR code sera automatiquement généré en inline (base64) dans le PDF.

**Méthode 2: API Programmatique**

```javascript
import { QRCodeService } from './src/qr-service.js';

const qrService = new QRCodeService();

// Générer HTML inline
const html = await qrService.generateInlineHTML('https://example.com');

// Générer style élégant
const elegantHTML = await qrService.generateElegantHTML('https://example.com');

// Traiter données template automatiquement
const data = { 'qr-code-url': 'https://...' };
const processed = await qrService.processTemplateData(data, true);
```

### Options de Génération

**Formats supportés:**

- PNG (défaut)
- SVG (vectoriel)
- Data URL (base64 inline)

**Niveaux de correction d'erreur:**

- L: ~7% récupération
- M: ~15% récupération
- Q: ~25% récupération
- H: ~30% récupération (recommandé)

**Couleurs prédéfinies:**

- Standard: noir/blanc
- Élégant: `#2c3e50` / `#fdfbf7` (template Christmas)

### Docker Usage

```bash
# Build
task qr:docker:build

# Generate
task qr:docker:run URL="https://example.com"

# Ou directement
docker run --rm -v $(pwd)/output:/app/output qr-generator \
  elegant -d "https://example.com" -o /app/output/qr.png
```

## Résolution de Problèmes

### Letter Generator - "socket hang up" Error

**Problème:** Erreur "socket hang up" lors de la génération de PDF.

**Cause:**

- Chromium browser non installé ou version incorrecte pour Puppeteer
- Versions obsolètes de Puppeteer (< 24.x) ne sont plus supportées

**Solution:**

1. Mettre à jour Puppeteer vers version 24+ dans `package.json`:

   ```json
   "puppeteer": "^24.2.0"
   ```

2. Installer le navigateur Chrome correct:

   ```bash
   cd apps/letter-generator
   npm install
   npx puppeteer browsers install chrome
   ```

3. Vérifier l'installation:
   ```bash
   ls ~/.cache/puppeteer/chrome/
   ```

**Configuration Puppeteer:**

- Browser installé dans: `~/.cache/puppeteer/`
- Version Chrome requise: 143.0.7499.40 (pour Puppeteer 24.x)
- Arguments de lancement optimisés pour macOS ARM64
- Gestion des ressources externes avec timeout et error handling

## Static Site Generator Usage

### Génération Rapide

```bash
# Générer le site de présentation
task site:generate

# Ouvrir dans le navigateur
task site:open

# Servir localement avec hot reload
task site:serve
```

### Structure du Site Généré

Le site contient :

- **Header avec H1 personnalisé** (depuis JSON)
- **Section "Ma Démarche"** avec explication détaillée
- **Workflow avec screenshot** + étapes détaillées
- **Matching avec l'entreprise** (points personnalisés)
- **Statistiques** en chiffres
- **CV intégré** via iframe
- **Footer** avec liens sociaux

### Personnalisation JSON

```json
{
  "main-title": "Votre titre H1 ici",
  "company-name": "Nom de l'entreprise",
  "workflow-image": "URL ou chemin vers screenshot",
  "workflow-steps": [...],
  "matching-points": [...],
  "cv-url": "https://votre-cv.com"
}
```

### Workflow Complet

```bash
# 1. Générer site de présentation
task site:generate

# 2. Déployer sur Netlify/Vercel
cd apps/static-site/output && netlify deploy

# 3. Générer QR code vers le site
task qr:generate:elegant URL="https://votre-site.com"

# 4. Générer lettre avec QR code
task letter:generate:qr

# Résultat : Lettre élégante + QR code → Site de présentation complet
```

### Sections Personnalisables

**Ma Démarche:**

- Expliquez votre approche automatisée
- Support HTML pour formatage riche

**Workflow:**

- Screenshot/diagramme du workflow
- Étapes numérotées avec descriptions

**Matching:**

- 4 points clés de matching avec icônes
- Personnalisé par entreprise

**Stats:**

- Chiffres clés (expérience, projets, etc.)
- Grid responsive automatique

**CV:**

- iframe vers CV en ligne
- Lien externe pour nouvel onglet

## Static Site - Style Synthwave

### Nouveau Template Rétro-Futuriste

**Style synthwave** avec :

- 🎨 Couleurs néon adoucies (rose/cyan/violet)
- 🌊 Grille 3D animée en fond
- ✨ Effets glow subtils
- 📱 Design responsive
- ⚡ Ultra performant

### Gestion Dynamique par URL

```bash
# Sans paramètre → Landing page (pas de matching)
https://job-search.draw-me-the-moon.fr

# Avec paramètre → Page personnalisée avec matching
https://job-search.draw-me-the-moon.fr?company=Hones
```

Le JavaScript détecte automatiquement le paramètre `company` et affiche/cache la section matching.

### Génération

```bash
# Style synthwave
task site:generate:synthwave

# Style élégant classique
task site:generate

# Ouvrir dans le navigateur
task site:open

# Servir localement (pour tester avec ?company=X)
cd apps/static-site
npx serve output -p 3000
open http://localhost:3000?company=Hones
```

### Images et CV

**Pour l'image workflow:**

- Créez votre diagramme sur Excalidraw/Figma
- Placez dans `assets/images/workflow.png`
- Ou utilisez un placeholder: `placehold.co`

**Pour le CV (iframe):**

- ✅ **Recommandé:** GitHub Pages (`username.github.io`)
- ✅ Google Drive (mode preview)
- ✅ Notion page publique
- ❌ Éviter: GitHub.com direct, LinkedIn (bloquent iframe)

Voir `apps/static-site/IMAGES-CV.md` pour le guide complet.

## Workflow API - Génération Complète de Candidature

### Vue d'ensemble

Le Workflow API permet de générer automatiquement tous les éléments d'une candidature professionnelle :

1. **Site de présentation** (HTML + PDF)
2. **QR code** pointant vers le site
3. **Lettre de motivation PDF** avec QR code intégré
4. **Hébergement self-hosted** du site HTML

### Architecture

**Backend-Orchestrated Workflow:**

- Une seule requête API pour tout le workflow
- Traitement asynchrone avec progress tracking
- Polling frontend pour récupérer le statut
- Stockage persistant en PostgreSQL

**Endpoints:**

```
POST   /api/workflow/application        # Créer un workflow
GET    /api/workflow/application/:id/status  # Récupérer le statut
GET    /hosted-sites/:siteName          # Servir le site HTML
```

### Utilisation

#### 1. Créer un Workflow

**Request:**

```bash
curl -X POST http://localhost:3001/api/workflow/application \
  -H "Content-Type: application/json" \
  -d '{
    "personalInfo": {
      "name": "Jean Dupont",
      "email": "jean.dupont@example.com",
      "phone": "+33 6 12 34 56 78",
      "linkedin": "https://linkedin.com/in/jeandupont",
      "github": "https://github.com/jeandupont"
    },
    "companyInfo": {
      "name": "Entreprise Cible",
      "position": "Développeur Full Stack Senior",
      "jobId": "uuid-optional"
    },
    "letterContent": {
      "template": "elegant",
      "introduction": "Passionné par le développement...",
      "motivation": "Ce qui m'\''a particulièrement interpellé...",
      "closing": "Je serais ravi d'\''échanger..."
    },
    "siteContent": {
      "template": "elegant",
      "title": "Jean Dupont",
      "subtitle": "Développeur Full Stack Senior",
      "about": "10 ans d'\''expérience...",
      "matchingPoints": [
        {
          "icon": "🚀",
          "title": "Innovation",
          "description": "Expertise en technologies modernes"
        }
      ],
      "stats": [
        { "number": "10+", "label": "Années d'\''expérience" },
        { "number": "50+", "label": "Projets livrés" }
      ]
    },
    "options": {
      "qrStyle": "elegant"
    }
  }'
```

**Response:**

```json
{
  "success": true,
  "workflowId": "uuid-workflow-id",
  "status": "processing",
  "progress": 0
}
```

#### 2. Polling du Statut

**Request:**

```bash
curl http://localhost:3001/api/workflow/application/:id/status
```

**Response (en cours):**

```json
{
  "success": true,
  "workflowId": "uuid-workflow-id",
  "status": "processing",
  "progress": 70
}
```

**Response (terminé):**

```json
{
  "success": true,
  "workflowId": "uuid-workflow-id",
  "status": "completed",
  "progress": 100,
  "files": {
    "siteHtml": "/hosted-sites/site-1234567890",
    "sitePdf": "/api/generators/site/download/site-1234567890.pdf",
    "qrImage": "/api/generators/qr/download/qr-1234567890.png",
    "letterPdf": "/api/generators/letter/download/letter-1234567890.pdf"
  }
}
```

#### 3. Accéder aux Fichiers

**Site HTML (self-hosted):**

```bash
# Ouvrir dans le navigateur
open http://localhost:3001/hosted-sites/site-1234567890
```

**Télécharger les PDFs:**

```bash
# Site PDF
curl -O http://localhost:3001/api/generators/site/download/site-1234567890.pdf

# Lettre PDF
curl -O http://localhost:3001/api/generators/letter/download/letter-1234567890.pdf

# QR Code PNG
curl -O http://localhost:3001/api/generators/qr/download/qr-1234567890.png
```

### Étapes du Workflow

Le workflow s'exécute en 6 étapes avec progress tracking :

1. **Génération site HTML** (0% → 20%)
   - Template hydraté avec données personnalisées
   - Sauvegarde dans `apps/static-site/output/` et `hosted-sites/`

2. **Génération site PDF** (20% → 40%)
   - Conversion HTML → PDF avec Puppeteer
   - Format A4, print-optimized

3. **Création URL site** (40% → 50%)
   - URL self-hosted : `/hosted-sites/site-{timestamp}`
   - Accessible immédiatement via backend

4. **Génération QR code** (50% → 70%)
   - QR pointant vers le site self-hosted
   - Style élégant avec couleurs personnalisées
   - Data URL (base64) pour intégration

5. **Génération lettre avec QR** (70% → 90%)
   - QR code intégré directement dans le PDF
   - Template élégant avec polices premium
   - Mise en page professionnelle

6. **Finalisation** (90% → 100%)
   - Sauvegarde résultats en DB
   - Status → "completed"
   - Fichiers prêts au téléchargement

### Templates Disponibles

**Letter Templates:**

- `elegant`: Template élégant de Noël (polices manuscrites, couleurs raffinées)
- `standard`: Template professionnel classique
- `christmas`: Alias pour elegant

**Site Templates:**

- `elegant`: Design classique et professionnel
- `synthwave`: Style rétro-futuriste avec néons

### Options de Personnalisation

**QR Style:**

- `elegant`: Couleurs assorties au template (#2c3e50 / #fdfbf7)
- `standard`: Noir et blanc classique

**Letter Data Fields:**

- `sender-name`, `sender-email`, `sender-phone`
- `company-name`, `position`
- `intro-paragraph`, `matching-description`, `closing-paragraph`
- `qr-code`: Auto-généré par le workflow
- `Date`: Auto-généré (date actuelle)

**Site Data Fields:**

- `title`, `subtitle`, `about`
- `workflow-steps`: Array d'étapes avec titre et description
- `matching-points`: Array de points avec icône, titre, description
- `stats`: Array de statistiques avec nombre et label
- `footer-links`: Array de liens avec URL et texte

### Gestion des Erreurs

**Status "failed":**

```json
{
  "success": false,
  "workflowId": "uuid",
  "status": "failed",
  "progress": 40,
  "error": "Message d'erreur détaillé"
}
```

**Erreurs communes:**

- Template inexistant
- Données JSON invalides
- Puppeteer timeout (images lourdes)
- Permissions fichiers (hosted-sites/)

### Base de Données

**Table `workflows`:**

- `id`: UUID
- `status`: 'pending' | 'processing' | 'completed' | 'failed'
- `progress`: 0-100
- `currentStep`: Nom de l'étape en cours
- `inputData`: JSON complet de la requête
- `result`: Chemins des fichiers générés + URL site
- `error`: Message d'erreur si failed
- `relatedJobId`: FK vers job scrapé (optionnel)
- `createdAt`, `updatedAt`: Timestamps

### Configuration

**Variables d'environnement (.env):**

```env
BASE_URL=http://localhost:3001
HOSTED_SITES_PATH=../hosted-sites
```

**Production:**

```env
BASE_URL=https://api.example.com
HOSTED_SITES_PATH=/var/www/hosted-sites
```

### Frontend Integration (À venir - Sprint 2)

**Wizard 6 étapes:**

1. Informations personnelles
2. Informations entreprise
3. Design du site
4. Contenu lettre
5. Prévisualisation
6. Résultat & téléchargements

**Intégrations:**

- Bouton sur chaque job scrapé : "Postuler avec générateur"
- Page standalone : "Créer une candidature"

### Self-Hosted Sites

**Fonctionnement:**

- Sites servis directement par le backend NestJS
- Endpoint dédié : `GET /hosted-sites/:siteName`
- Fichiers HTML stockés dans `hosted-sites/`
- Pas de déploiement externe requis (Netlify/Vercel)

**Avantages:**

- Hébergement instantané (pas de build/deploy)
- URLs immédiates pour QR codes
- Contrôle total sur les fichiers
- Pas de coûts externes

**Limitations:**

- Requiert backend running pour accès
- Pas de CDN (pour MVP)
- Scalabilité limitée vs cloud hosting

### Exemple Complet

```bash
# 1. Créer le workflow
WORKFLOW_ID=$(curl -s -X POST http://localhost:3001/api/workflow/application \
  -H "Content-Type: application/json" \
  -d @workflow-data.json | jq -r '.workflowId')

echo "Workflow créé: $WORKFLOW_ID"

# 2. Polling du statut (toutes les 2 secondes)
while true; do
  STATUS=$(curl -s http://localhost:3001/api/workflow/application/$WORKFLOW_ID/status)
  CURRENT_STATUS=$(echo $STATUS | jq -r '.status')
  PROGRESS=$(echo $STATUS | jq -r '.progress')

  echo "Status: $CURRENT_STATUS - Progress: $PROGRESS%"

  if [ "$CURRENT_STATUS" = "completed" ] || [ "$CURRENT_STATUS" = "failed" ]; then
    break
  fi

  sleep 2
done

# 3. Récupérer les URLs
if [ "$CURRENT_STATUS" = "completed" ]; then
  echo $STATUS | jq '.files'

  # Ouvrir le site
  SITE_URL=$(echo $STATUS | jq -r '.files.siteHtml')
  open "http://localhost:3001$SITE_URL"
fi
```

## Dernière Mise à Jour

Date: 2025-12-11
Version: 1.9.0
Status: Interface de gestion des URLs implémentée avec succès.

**Nouvelles fonctionnalités:**

- ✅ Frontend redesigné avec formulaire de gestion des URLs
- ✅ Ajout d'URLs avec génération automatique du nom à partir de l'URL
- ✅ Validation des doublons (impossible d'ajouter la même URL deux fois)
- ✅ Affichage de la liste complète des URLs avec détails (nom, URL, date de création, dernière exécution)
- ✅ Backend complet activé avec tous les modules (Scrapers, Jobs, Generators, Workflow, Hosting)
- ✅ API complète disponible sur http://localhost:3001/api
- ✅ Documentation Swagger disponible sur http://localhost:3001/api

**Services opérationnels:**

- Frontend: http://localhost:5174 (Interface de gestion des URLs)
- Backend: http://localhost:3001 (API complète)
- PostgreSQL + Redis running
- Tous les modules backend actifs

Workflow API complet implémenté - Sprint 1 terminé. Backend fonctionnel avec orchestration complète Site → QR → Letter. Frontend wizard à implémenter (Sprint 2).
